...
function FileSharing() { //this function is for our file sharing app
 ...
 <div class="flex space-x-4 -mt-1 -ml-185">
      ...
      hover:from-[#4198b7] hover:to-[#21295b] 
        active:scale-95 " ${
            selectedTab() === "URL SHORTENER" ? "bg-[#3a675d] text-black" : "bg-[#9ba4c8] text-black font-bold transition-all duration-300 hover:sclae-105 hover:shadow-xl hover:from-[#4198b7] hover:to-[#21295b] active:scale-95 "
          }`}
          onClick={() => setSelectedTab("URL SHORTENER")}
        >
          URL SHORTENER
        </button>
        <button
          class={`px-6 py-3 rounded-[20px] text-xl uppercase font-bold shadow-lg transition-all duration-300 
        hover:scale-105 hover:shadow-xl 
      hover:from-[#4198b7] hover:to-[#21295b] 
        active:scale-95   ${
      ...
      <Show when={selectedTab() === "URL SHORTENER"}>
  <div 
    class="text-white 5 mt-4 flex flex-col items-center -ml-176 bg-[#0a0f1f] rounded-4xl w-2/3 max-w-lg transition-all p-4 "
    style={{ height: urlShorten() ? 'auto' : '28rem' }} // Expands dynamically
  >
    <img class="w-60 h-auto rounded-4xl -ml-5 pb-2" loading="lazy" src={url} alt="url" />
    <span class="text-cyan-200 pb-5 -ml-1 mt-3 text-3xl [text-shadow:0px_0px_1px_rgb(98,228,255),0px_0px_2px_rgb(98,228,255)]">Cut the Clutter, Keep the Clicks ! </span>
      ...
      <Show when={selectedTab() === "FILE SHARING"}>
        <div class="w-2/3 max-w-lg bg-[#0a0f1f] p-4 rounded-4xl transition-all -ml-173 mt-4 pointer-events-auto"
          ...
          <div class="w-full h-40 border-2  border-dashed border-cyan-200 rounded-lg flex flex-col items-center justify-center text-gray-300 hover:border-blue-400 transition-all cursor-pointer">
            <p class="text-2xl pb-2 tracking-wide text-yellow-200 ">Drop your files here.</p>
    ...
    <div class="absolute w-190 h-130 bg-[radial-gradient(circle,_rgba(80,194,204,0.6)_10%,_rgba(0,14,51,0)_90%)] blur-2xl opacity-30 pointer-events-none hidden lg:flex"
       style={{ top: "50%", left: "75%", transform: "translate(-50%, -50%)" }}></div>
    </div>
    </>
  );
}
